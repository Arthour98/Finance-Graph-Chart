  ...
  return (
    ...
    >
      <div className="sticky top-0 bg-black p-2 md:p-4 flex flex-col sm:flex-row gap-2 sm:gap-4">
        ...
        >
          {months.map((month, index) => {
            return (
              <option className="text-black" key={index} value={month}>
                {month}
              </option>
            );
          })}
      ...
      <div className="flex-1 w-full overflow-y-scroll scrollbar-hide">
        {/* Desktop Table View */}
        <table
          className="hidden md:table border-separate border-gray-600
                w-full border-spacing-y-2 md:border-spacing-y-2 table-auto shadow-md "
        >
          <thead className="sticky top-0 bg-black  ">
            <tr>
              <th className="border border-gray-600 p-2 text-xs md:text-sm">
                Month
              </th>
              <th className="border border-gray-600 p-2 text-xs md:text-sm">
                Expense-Description
                ...
                <tr key={index}>
                  ...
                  <td className="p-1">
                    <div className="flex justify-center">
                      <button
                        onClick={() =>
                          selTransaction?.length > 0 &&
                          selTransaction[0]?.id == transaction?.id
                            ? setSelTransaction([])
                        ...
                        className={`${
                          ...
                          selTransaction[0]?.id == transaction?.id
                            ? `bg-cyan-500`
                            : `bg-amber-500`
                        } cursor-pointer min-w-[60%] py-1 px-3 md:py-2
                  ...
                  <td className="p-1">
                    <div className="flex justify-center">
                      <button
                        className="bg-red-500 cursor-pointer py-1 px-3 md:py-2 md:px-4 rounded-4xl text-xs md:text-sm"
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-2 justify-center">
                  <button
                    ...
                    className={`${
                      ...
                      selTransaction[0]?.id == transaction?.id
                        ? `bg-cyan-500`
                        : `bg-amber-500`
                    } cursor-pointer min-w-[50%] py-1 px-3 md:py-2
                                             md:px-4 rounded-4xl text-xs md:text-sm`}
                  >
                    {selTransaction?.length > 0 &&
                    selTransaction[0]?.id == transaction?.id
                  ...
                  <button
                    className="bg-red-500 cursor-pointer py-1 px-3 rounded-4xl text-xs flex-1"
                    onClick={() => deleteTransaction(transaction.id)}
                  >
                    Delete
                  </button>
